...
CREATE TABLE `COURSE` (
    ...
    `Year` int(5) NOT NULL,
    `CourseID` int(11) NOT NULL UNIQUE,
    PRIMARY KEY(`CourseID`)
);


INSERT INTO `STUDENT` VALUES('John', 'Dougherty', 'Computer Science', 1234);
INSERT INTO `STUDENT` VALUES('Jane', 'Doe', 'Management Science', 5678);
INSERT INTO `STUDENT` VALUES('Earl', 'Baker', 'Sumty Sumn', 3456);
INSERT INTO `STUDENT` VALUES('Jimmy', 'Quoyang', 'Microbio', 4590);
...
FROM ASSIGNMENT a, SCORE s
WHERE a.AssignmentID=2 AND s.AssignmentID=a.AssignmentID;

-- TASK 5
-- List all students of a given course
-- OPTION 1
SELECT s.FirstName
FROM STUDENT s
WHERE s.StudentID IN (
  select e.StudentID from ENROLLMENT e where e.CourseID=85675
);
...
UPDATE DISTRIBUTION SET Percentage = 40 WHERE CategoryName = 'Quiz';

-- TASK 9
-- Add 2 points to the score of each student on an assignment
UPDATE SCORE SET POINTS = POINTS + 2 WHERE AssignmentID = 1;

-- TASK 10
-- Add 2 points just to those students whose last name contains a ‘Q’
UPDATE SCORE
SET POINTS = POINTS + 2
WHERE SCORE.StudentID = (
  SELECT StudentID
  FROM STUDENT
  WHERE SCORE.StudentID = STUDENT.StudentID AND STUDENT.LastName LIKE '%Q%'
);

-- TASK 11
-- Compute the grade for a student
SELECT SUM((G.POINTS) * (D.Percentage / C.COUNTER)/PointsPossible) AS FINAL_GRADE FROM SCORE G
LEFT JOIN ASSIGNMENT A ON G.AssignmentID = A.AssignmentID
JOIN DISTRIBUTION D ON D.DistributionID = A.DistributionID
JOIN (
  SELECT D.DistributionID, COUNT(*) AS COUNTER
  FROM SCORE G LEFT JOIN ASSIGNMENT A
  ON G.AssignmentID = A.AssignmentID JOIN DISTRIBUTION D
  ON D.DistributionID = A.DistributionID
  WHERE CourseID=90573 AND StudentID=5678
...
JOIN (
  ...
  JOIN DISTRIBUTION D ON D.DistributionID = A.DistributionID
  WHERE CourseID=90573 AND StudentID=5678 GROUP BY D.DistributionID
